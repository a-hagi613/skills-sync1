...
export default function SideNav() {
  ...
  async function fetchResumes() {
    setIsLoading(true);
    const url =
      `https://skill-sync.vercel.app/api/resume/${userId}`;

      //`http://127.0.0.1:3000/api/resume/${userId}`;
    if (
      userId !== null &&
      userId !== undefined &&
      isSignedIn === true &&
      ...
      >
        ...
        <div>
          {resumes.map((resume, index) => (
            <div key={resume.id} className="p-1 my-3">
              <p className="font-bold text-md">Resume #{resume.id}</p>
              <p className="mb-2 ml-2 truncate">{resume.originalText}</p>
              <button
                onClick={() => {
                  setIsModalOpen(true);
          ...
          <div className="w-3/4 p-4 bg-white rounded-lg shadow-2xl shadow-gradient">
            <div className="flex items-start justify-between">
              <ReactMarkdown className="p-5 leading-loose text-gray-800 text-md">
                {resumes[selectedResumeIndex]?.generatedText}
              </ReactMarkdown>
              <div>
                <CopyToClipboard
                  text={markdownToText(
